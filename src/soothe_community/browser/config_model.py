"""Typed YAML for the browser community subagent (IG-415)."""
...
class BrowserSubagentConfig(BaseModel):
    ...
    Args:
        ...
        max_steps: Maximum browser automation steps per delegated task (browser-use
            loop). Override via YAML ``subagents.browser.config.max_steps``.
    """

    max_steps: int = Field(default=10, ge=1, description="Maximum browser automation steps per task.")
    runtime_dir: str = ""
    downloads_dir: str = ""
    user_data_dir: str = ""
    extensions_dir: str = ""
